...
class Transformer(nn.Module):
    def __init__(self, args, img_size=224, patch_size=16, in_chans=3, num_classes=10, embed_dim=384, depth=[2,2,2,2],
        ...
        if args.ln is not None:
            print('You choose layer norm')
            self.norm1 = CustomNorm('ln', embed_dim*2)
            self.norm2 = CustomNorm('ln', embed_dim)
            self.norm3 = CustomNorm('ln', embed_dim // 2)
            self.norm4 = CustomNorm('ln', embed_dim // 4)

        if args.fusconv is not None:
            print('You choose fusconv')
            self.fuscov1 = nn.Conv2d(embed_dim*2, embed_dim*2, 1)
            self.fuscov2 = nn.Conv2d(embed_dim, embed_dim, 1)
            self.fuscov3 = nn.Conv2d(embed_dim//2, embed_dim//2, 1)
            self.fuscov4 = nn.Conv2d(embed_dim // 4, embed_dim // 4, 1)
    ...
    def forward(self, x, gsfeatures, inirgb):
        ...
        if self.args.fusconv is not None:
            x = self.fuscov1(x)
        x,C,H,W = cnn2tf(x)
        if self.args.ln is not None:
            x = self.norm1(x)
